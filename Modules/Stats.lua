...
local function addonTooltip(self)
    ...
    for _, ntry in pairs(addons) do
        if nr < addonList then
            GameTooltip:AddDoubleLine(ntry.name, memFormat(ntry.memory), 1, 1, 1, RGBGradient(ntry.memory / 800))
            nr = nr + 1
        end
    end
    GameTooltip:AddLine(" ")
...
local function update(self, elapsed)
    ...
    if lastUpdate > 1 then
        ...
        if showClock == true then
            StatsFrame.text:SetText(getFPS() .. " " .. getLatencyWorld() .. " " .. getMail() .. " " .. getTime())
        else
            StatsFrame.text:SetText(getFPS() .. " " .. getLatencyWorld() .. " " .. getMail())
        end
