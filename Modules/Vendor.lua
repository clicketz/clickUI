local function OnEvent(self, event)
    ...
    if totalPrice ~= 0 then
        DEFAULT_CHAT_FRAME:AddMessage("Items were sold for " .. C_CurrencyInfo.GetCoinTextureString(totalPrice), 255, 255, 255)
    end
    --[[
    -- Auto Repair
    ...
    if (CanMerchantRepair()) then
        ...
        -- If merchant can repair and there is something to repair
        if (repairAllCost and canRepair and repairAllCost > 0) then
            --Guild funds first
            RepairAllItems(true)
            RepairAllItems(false)
            DEFAULT_CHAT_FRAME:AddMessage("Equipment has been repaired for " .. C_CurrencyInfo.GetCoinTextureString(repairAllCost), 255, 255, 255)
        end
    end
end
